...
_CREDENTIAL_RE = re.compile(
    ...
    r"|ghs_[A-Za-z0-9]{20,}"         # GitHub server-to-server tokens
    r"|Bearer\s+[A-Za-z0-9._-]{20,}" # Bearer tokens in error messages
    r"|key=[A-Za-z0-9_-]{20,})"      # URL query string keys
)


def _scrub_credentials(obj: object) -> object:
    """Recursively scrub credential patterns from all strings in a nested structure."""
    if isinstance(obj, str):
        return _CREDENTIAL_RE.sub("[REDACTED]", obj)
    if isinstance(obj, dict):
        return {k: _scrub_credentials(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_scrub_credentials(item) for item in obj]
    return obj
...
def build_manifest(table: dict, base_url: str = DEFAULT_BASE_URL, provider_access_runtime: dict | None = None) -> dict:
    ...
    manifest = {
        ...
        "observability": _scrub_credentials({
    ...
    manifest = _scrub_credentials(manifest)
